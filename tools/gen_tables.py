...
def generate_combined():
    """Combined LUTs for the one-RBF Buck Rogers/Turbo strap (WORKPLAN_TURBO_GRAPHICS.md
    Step 1). $readmemh can't be conditional on a runtime strap, so both games' tables are
    loaded into a single BRAM at elaboration and selected at runtime by indexing on
    mod_turbo, the strap's MSB.

    xscale_combined.hex: [0:511], {mod_turbo, dacinput} -- buckrog 0-255, turbo 256-511.
    palette_combined.hex: [0:2047], {mod_turbo, palbits} -- buckrog 0-1023 (10-bit palbits,
    already zero-extended to 11 bits by the mod_turbo MSB), turbo 1024-1279 (8-bit pen,
    zero-extended to 11 bits), remainder zero.
    """
    written = []

    xscale_words = BUCKROG.xscale_table() + TURBO.xscale_table()
    written.append(write_hex(
        ROMS / "xscale_combined.hex", xscale_words, 8,
        ["combined sprite X-scale step, Q8.24 fraction of the 2x pixel clock",
         "index {mod_turbo, dacinput}: 0-255 buckrog, 256-511 turbo",
         "generated by tools/gen_tables.py - do not edit"]))

    buckrog_palette = [(r << 16) | (g << 8) | b for r, g, b in BUCKROG.palette_table()]
    turbo_palette = [(r << 16) | (g << 8) | b for r, g, b in TURBO.palette_table()]
    palette_words = buckrog_palette + turbo_palette + [0] * (2048 - len(buckrog_palette) - len(turbo_palette))
    written.append(write_hex(
        ROMS / "palette_combined.hex", palette_words, 6,
        ["combined palette, 0xRRGGBB",
         "index {mod_turbo, palbits}: 0-1023 buckrog, 1024-1279 turbo, rest zero",
         "generated by tools/gen_tables.py - do not edit"]))
